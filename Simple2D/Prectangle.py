...
class Prectangle(PCloseShape):
    ...
    def tranlate(self,point):
        ...
        self.updatePath()
    def scaleM(self,S):
        self.rect = S.mapRect(rect)
        self.updatePath()


    def rotate(self,dTheta,ptOrigin):
        pt[4] = [QPoint() for i in range(4)]
        ...
        if fabs(dTheta)<M_PI_4:
            center=pt[0]
        elif fabs(dTheta)>M_PI_4 and fabs(dTheta)<3*M_PI_4:
            center=pt[1]
        elif fabs(dTheta)>3*M_PI_4:
            center=pt[2]
        else:
            center=pt[3]
        ...
        if fabs(dTheta)>M_PI_4 and fabs(dTheta)<3*M_PI_4 :
